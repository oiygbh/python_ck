...
def main():
    t1 = threading.Thread(target=func1)  # 指定线程1运行的函数
    t2 = threading.Thread(target=func2)  # 指定线程2运行的函数
    s_time = time.time()

    t1.setName('线程1')  # 设置线程名
    t2.setName('线程2')  # 设置线程名
    print("线程是否运行:",t1.is_alive())  # 线程是否执行
    t1.start()  # 开始执行线程1
    t2.start()  # 开始执行线程2
    print("线程是否运行:",t1.is_alive())  # 线程是否执行
    # 让主线程等待子线程执行完再继续往下执行
    ...
    t2.join()  # 等待主线程运行时间，默认为线程运行结束
    print("获取线程1的线程名:",t1.getName())
    e_time = time.time()
    print("获取线程2的线程名:",t2.getName())
    print("耗时：", e_time - s_time)
